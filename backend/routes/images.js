    ...
    Image.find(query).then((data) => {
      if (req.user === undefined) {
        ...
        User.findById(req.params.id).then((user) => {
          ...
          auth = false;
          res.render("profile", { title: name, auth, images: data });
        });
      }
    });
  } else {
...
router.get("/image/:id", isLoggedIn, (req, res) => {
  ...
  Image.findById(id).then((match) => {
    ...
    if (helper === true) {
      ...
      match.save();
      Image.update({ _id: id }, { $push: { likedBy: req.user._id } }, (err) => {
        if (err) console.log(err);
      });
      res.send([match.likedBy, match._id, match.likeClass]);
      User.findById(req.user._id).then((user) => {
        if (match.user.toString() !== req.user.id.toString()) {
          ...
          User.update(
            { _id: match.user },
            { $push: { "local.notifications": message } },
            (err) => {
              if (err) console.log(err);
            }
          );
...
router.get("/show/image/:id", (req, res) => {
  let query = req.params.id;
  let userId = [];
  let userName = [];
  let name;
  Image.findById(query).then((image) => res.status(200).json(image));

  // Image.findById(query).then((image) => {
  ...
  //         image.user === req.user._id.toString()
  //       ) {
  //         doc.auth = true;
  //       } else {
  //         doc.auth = false;
  //       }
  //     });
  //     Image.find({ category: image.category }).then((similar) => {
...
router.get("/categories/:name", (req, res) => {
  ...
  Image.find(query).then((images) => {
    if (req.user === undefined || req.user === null) {
      res.status(200).json(images);
    } else {
      images.map((doc) => {
        doc.likedBy.map((liked) => {
          if (liked.toString() === req.user.sub.toString()) {
            doc.likeClass = true;
            doc.save();
          } else {
            doc.likeClass = false;
