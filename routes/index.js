...
let mailer = nodemailer.createTransport(sgTransport(options));
     ...
     mailer.sendMail(email, function(err) {
        req.flash('success_msg', 'An e-mail has been sent to ' + user.local.email + ' with further instructions.Check also in spam folder');
        done(err, 'done');
      });
    }
...
var mailer = nodemailer.createTransport(sgTransport(options));
     ...
     mailer.sendMail(email, function(err) {
        req.flash('success_msg', 'An e-mail has been sent to ' + user.local.email + ' with further instructions.');
        done(err, 'done');
      });
    }
  ]);
